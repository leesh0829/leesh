...
| 정산 대기함(청구·비상금) | `app/ledger/settlements/*` | `/api/ledger/settlements` (GET) | `LedgerEntry.settlementKind/settlementStatus/settledAt` |
...
다음 월급에 정상화할 항목을 추적합니다. 청구·비상금은 **별도 테이블이 아니라 가계부 내역(`LedgerEntry`)의 정산 태그**입니다. `settlementKind`(`REIMBURSEMENT` 청구 — 개인카드 대납 / `EMERGENCY` 비상금 — 임시 인출, null이면 일반 내역)와 `settlementStatus`(`PENDING` 미정산 / `SETTLED` 정산완료)로 구분합니다. 청구·비상금은 **지출(EXPENSE) 전용**이고 실제 나간 돈이므로 **가계부 합계에 정상 반영**됩니다. 정산은 **상태만 전환**하며 별도 거래를 자동 생성하지 않습니다 — 실제 환급 수입/되갚기 이체는 사용자가 가계부에서 직접 기입합니다. **본인 전용**(공유 비대상).

- **모델 필드(`LedgerEntry`)**: `settlementKind SettlementKind?`·`settlementStatus SettlementStatus?`·`settledAt DateTime?`. 인덱스 `@@index([ownerId, settlementKind])`. (기존 `Settlement` 모델은 제거, enum은 유지·재사용.)
...
- **등록·수정은 가계부 내역에서**: `LedgerClient`의 `항목 추가`·인라인 수정 폼에 `일반 / 청구 / 비상금` 버튼. 청구·비상금 선택 시 지출로 고정. 내역 카드에는 정산 여부 배지(`청구 · 미정산` 등, 청구=sky·비상금=amber·정산완료는 흐리게)를 표시. 생성은 `POST /api/ledger`, 수정·태그 변경·상태 토글은 `PATCH /api/ledger/[entryId]`(`settlementKind`/`settlementStatus`) 재사용.
- **페이지** `app/ledger/settlements/page.tsx` → `SettlementsClient`: **조회 + 정산 여부 체크 전용**. 상단 요약(받을 청구/갚을 비상금 미정산 합계), 종류·상태 필터, 항목별 `[정산 완료]`/`[미정산으로]` 토글만(추가·수정·삭제 없음 — 안내 문구로 `/ledger`로 유도).
- **API** `GET /api/ledger/settlements?kind=&status=`: 태그 달린 `LedgerEntry`만 조회(미정산 우선 정렬) + `summary`. 생성/수정용 `POST`·`[id]` 라우트는 제거. `runtime='nodejs'`·본인 인증.
- **메인 `/ledger` 요약 카드**: `LedgerClient`가 `GET /api/ledger/settlements`의 `summary`를 불러와(마운트 시 + 정산 항목 생성·수정 후 갱신), 미정산 합계가 있을 때만 카드를 노출하고 클릭 시 이 페이지로 이동합니다. 네비 버튼 `LedgerNavSettlements`(`LedgerNavIcons.tsx`).
