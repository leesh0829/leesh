...
## 개정 (2026-07-21, 사용자 피드백)

1. **색상**: 참고 파일 팔레트(크림/잉크/오렌지) 대신 **기존 프로젝트 색상 가이드** 사용 —
   라이트 `#f7f8fb`/`#0b1020`/`#6d5aff`/`#26c6ff`, 다크 `#060712`/`#e7e9f3`/`#8c7bff`/`#32d1ff`.
   판형 구조(하드보더·노이즈·크롭마크·모노 레이블)는 유지.
2. **상단 버튼 유지**: 검색·테마(우상단, z-50)·홈(좌상단) 버튼이 sticky 네비에 가려지지 않도록
   `.leesh-nav`를 z-45로 낮추고 lg+에서 우측 118px 자리 확보.
3. **히어로 3D**: 스펙시트 → **코드 에디터 창 스택**(파일명 타이틀바 + 코드 라인 + 터미널 프롬프트, CSS 전용).
4. **`§` 접두 제거**: 네비 링크·섹션 번호는 `01`~`09` 숫자만 사용.
5. **푸터**: `/leesh`는 전용 `leesh-footer`만 표시 — AppShell `isLeeshPage` 분기에서 전역 `<Footer />` 제거
   (다른 페이지는 영향 없음).
